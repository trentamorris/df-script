...
console.log("=========================================");
console.log("STARTING COLUMN EXPRESSION LIST.EVAL TESTS...");
console.log("=========================================");
...
try {
    ...
    const dfArithmetic = df.select([
        $df.col("numbers").list.eval(element().mul(2)).alias("arithmetic")
    ]);
    ...
    console.log("✓ Basic list.eval element arithmetic passed");

    // 2. Aggregation: element().sum()
    const dfSum = df.select([
        $df.col("numbers").list.eval(element().sum()).alias("sum")
    ]);
    ...
    console.log("✓ list.eval element.sum() aggregation passed");

    // 3. Chained aggregation and post-ops: element().sum().mul(10).add(1)
    const dfChained = df.select([
        $df.col("numbers").list.eval(element().sum().mul(10).add(1)).alias("chained")
    ]);
    ...
    console.log("✓ list.eval chained aggregation & post-ops passed");

    // 4. Element-wise string: element().str.to_uppercase()
    const dfString = df.select([
        $df.col("tags").list.eval(element().str.to_uppercase()).alias("upper")
    ]);
    ...
    console.log("✓ list.eval element string operation passed");

    // 5. Null row handling robustness
    const dfNull = df.select([
        $df.col("null_col").list.eval(element().add(1)).alias("null_eval")
    ]);
    ...
    console.log("✓ list.eval null row robustness passed");

    // 6. Partitioning inside list.eval: element().sum().over(element())
    const dfOver = $df.data([
        { numbers: [1, 1, 2] }
    ]).select([
        $df.col("numbers").list.eval(element().sum().over(element())).alias("over_eval")
    ]);
    ...
    console.log("✓ list.eval window partitioning .over() passed");

    // 7. Custom partition expressions: element().sum().over(element().mod(2))
    const dfCustomOver = $df.data([
        { numbers: [1, 2, 3, 4] }
    ]).select([
        $df.col("numbers").list.eval(element().sum().over(element().mod(2))).alias("custom_over")
    ]);
    ...
    console.log("✓ list.eval custom partitioning expressions (odd/even) passed");

    console.log("\n🎉 ALL Expr.list.eval COLUMN EXPRESSION TESTS PASSED SUCCESSFULLY!");
} catch (err) {
    console.error("\n❌ Expr.list.eval COLUMN EXPRESSION TESTS FAILED:", err);
    process.exit(1);
}
