...
try {
    ...
    df.to_array("non_existent" as any);
...
try {
    ...
    df.select($df.col("a").arr.get(5, false)).to_dicts();
...
try {
    const df = $df.data([{ a: [1, 2, 3] }]);
    df.select($df.col("a").arr.gather_every({ step: 0 })).to_dicts();
...
try {
    ...
    df.select($df.col("a").arr.gather_every({ offsetStart: 5, null_on_oob: false })).to_dicts();
...
const result_oob_null = df_oob_null.select($df.col("a").arr.gather_every({ offsetStart: 5, null_on_oob: true }).alias("sliced")).to_dicts();
