...
const schemaSparse = dfSparse.schema;
...
const alignedSchema = dfAligned.schema;
...
assert(Object.keys(dfEmpty.schema).length === 2, "Height-0 should preserve original schema");
...
assert(Object.keys(dfEmptySelected.schema).length === 1 && "a" in dfEmptySelected.schema, "Select on height-0 should preserve schema selection");
...
assert(Object.keys(dfEmptyDropped.schema).length === 1 && "b" in dfEmptyDropped.schema, "Drop on height-0 should preserve schema");
...
assert(Object.keys(dfEmptyRenamed.schema).length === 2 && "c" in dfEmptyRenamed.schema, "Rename on height-0 should preserve schema");
...
assert(Object.keys(dfEmptyWithCols.schema).length === 4, "With_columns on height-0 should append new columns to schema");
...
const unpivotedSchema = dfEmptyUnpivoted.schema;
