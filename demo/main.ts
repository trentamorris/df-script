...
// Preset Queries
const PRESETS: Record<string, { dataset: DatasetName; query: string }> = {
  filter: {
    dataset: "sales",
    query: `df.filter(\n  $df.col("sales").gt(100)\n)`
  },
...
// State
let activeDf: DataFrame = $df.data(MOCK_DATASETS.sales);
let currentResultDf: DataFrame = activeDf;
...
const statShape = document.getElementById("stat-shape") as HTMLSpanElement;
const resultTable = document.getElementById("result-table") as HTMLTableElement;

// Buttons
const btnParseCsv = document.getElementById("btn-parse-csv") as HTMLButtonElement;
const btnRun = document.getElementById("btn-run") as HTMLButtonElement;
const btnCopyCsv = document.getElementById("btn-copy-csv") as HTMLButtonElement;
...
function updateActiveDataset() {
  const datasetName = datasetSelect.value as DatasetName;
...
function getTypeBadgeClass(typeStr: string): string {
  ...
  if (name.includes("int") || name.includes("uint")) return "type-int";
  if (name.includes("float") || name.includes("decimal")) return "type-float";
  if (name.includes("utf8") || name.includes("string")) return "type-utf8";
  if (name.includes("bool")) return "type-bool";
  if (name.includes("date") || name.includes("time")) return "type-datetime";
  return "type-utf8";
}
 ...
 */
function renderTable(df: DataFrame) {
  const schema = df.get_schema();
  const cols = df.columns;
  const data = df.to_dicts() as any[];
...
document.querySelectorAll(".btn-preset").forEach(btn => {
  btn.addEventListener("click", (e) => {
    const presetName = (e.currentTarget as HTMLButtonElement).dataset.preset!;
